...
class Node(object):
    ...
    def refresh_node_load(self):
        ...
        print('Refreshed node {0} to load {1}'.format(self.node_name, self.current_load))
        time.sleep(5)

    def __str__(self):
        return 'Node {0} IP = {1} Load = {2}'.format(self.node_name, self.ipaddress, self.current_load)
